...
interp :: Expr -> Int
interp (Lit n) = n
interp (Op binOp expr1 expr2) = interpBinOp binOp (interp expr1) (interp expr2)
...
simplifyZero (Op Plus (Lit 0) expr) = expr
simplifyZero (Op Plus expr (Lit 0)) = expr
simplifyZero expr = expr
...
peephole opt (Op binOp expr1 expr2) = opt (Op binOp (peephole opt expr1) (peephole opt expr2))
peephole opt (Lit n) = Lit n
...
step (IOp binOp) (y : x : stack) = Just (interpBinOp binOp x y : stack)
step _ _ = Nothing
...
run instructions stack = foldM (\stk instr -> step instr stk) stack instructions
...
compile (Op binOp expr1 expr2) = compile expr1 ++ compile expr2 ++ [IOp binOp]
