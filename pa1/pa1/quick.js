...
const partition = (g, ls) => List([filter(g, ls), filter(x => !g(x), ls)]);
...
const quicksort = function (ls) {
  ...
  ls = ls.shift();
  const parts = partition((x) => x <= pivot, ls);
  const arr1 = quicksort(parts.get(0));
  const arr2 = quicksort(parts.get(1));
  return arr1.concat(List([pivot])).concat(arr2);
};
